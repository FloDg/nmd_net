...
class InverseAdaptiveAPLLayer():
    def __init__(self, supervisor, previous_size, size, params):
        ...
        self.adapt_features_size = previous_size[2]
        self.size = size
    ...
    def __call__(self, list_of_inputs):
        ...
        prev_output = list_of_inputs[1]
        adapt_features = list_of_inputs[2]
        prev_layer_input = tf.matmul(inp, self.w) + self.b
        x = prev_output
        ...
        f_output = tf.add_n(linear_pieces)
        output = prev_output + prev_layer_input - f_output
        return [output]
