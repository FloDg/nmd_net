...
class KlLayer():
    ...
    def __call__(self, list_of_inputs):
        ...
        log_det_cov_old = tf.reduce_sum(input_tensor=old_log_vars, axis = 1)
        log_det_cov_new = tf.reduce_sum(input_tensor=log_vars, axis = 1)
        tr_old_new = tf.reduce_sum(input_tensor=tf.exp(old_log_vars - log_vars), axis = 1)
        ...
        kl = tf.reshape(0.5 * (log_det_cov_new - log_det_cov_old + tr_old_new +\
             tf.reduce_sum(input_tensor=tf.square(means - old_means) /
